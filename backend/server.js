...
const dbConfig = {
  ...
  password: process.env.DB_PASSWORD || 'your_mysql_password', // 실제 MySQL 비밀번호로 변경하세요.
  database: process.env.DB_NAME || 'todo', // 당신의 데이터베이스 이름 'todo'로 변경
...
const logActivity = async (userId, projectId, taskId, behavior, details) => {
  ...
  try {
    ...
    connection.release();
    console.log(`Activity logged: ${behavior}`);
  } catch (err) {
